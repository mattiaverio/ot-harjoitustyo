...
class MenuPage(tk.Frame):
    def __init__(self, master, ui):
        ...
        tk.Button(self, text="Start Game", command=self.start_game_click).pack(pady=10)
    ...
    def start_game_click(self):
        white_name = simpledialog.askstring("Input", "Username for White pieces:", parent=self)
        if not white_name: return 

        black_name = simpledialog.askstring("Input", "Username for Black pieces:", parent=self)
        if not black_name: return
